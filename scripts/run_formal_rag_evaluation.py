...
def run_formal_evaluation(tenant_id: str, top_k: int = 10) -> dict:
    ...
    for name, path in (("title", GOLD_PATH), ("colloquial", COLLOQUIAL_PATH)):
        metrics, outcomes, latencies = evaluate_dataset(
            load_jsonl(path),
            access=access,
            index_root=root,
        ...
        retrieval[name] = {
            ...
            "latency_ms": {
                mode: {
                    "count": len(values),
                    "mean": sum(values) / len(values) * 1000 if values else 0.0,
                    "max": max(values) * 1000 if values else 0.0,
                }
                for mode, values in latencies.items()
            },
            "details": [{"id": item.case_id, "ranks": item.ranks} for item in outcomes],
        }
    ...
    metadata = [
        {
            key: case.get(key, "")
            for key in (
                "id", "scenario", "case_type", "expected_intent",
                "expected_evidence_keywords", "forbidden_keywords", "notes",
            )
        }
        for case in cases
    ]
    ...
    def answer_provider(query: str) -> dict:
        request = SimpleNamespace(message=query, user_id="formal-eval", channel="evaluation")
        return get_answer_from_rag(request, auth=auth)

    reports = build_grounding_reports_from_rag(
        [case["query"] for case in cases], answer_provider, metadata
    )
    return aggregate_formal_report(
        {"tenant_id": tenant_id, "index": index, "datasets": retrieval},
        {"summary": summarize_grounding_reports(reports), "reports": reports},
    )


def main() -> int:
    parser = argparse.ArgumentParser(description="Run formal chunk RAG evaluation in one command.")
